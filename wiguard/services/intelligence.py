...
EXTRACTED_LIST_KEYS = {
    ...
    "companion_exports", "native_source_manifest", "binary_evidence_summary",
    ...
    "universal_json_preview", "external_converter_outputs",
}


REAL_OBJECT_KEYS = {
    "devices", "interfaces", "vlans", "dhcp_scopes", "dhcp_excluded",
    "acl_rules", "nat_rules", "cdp_links", "lldp_links", "services",
    "ip_inventory", "interface_status", "trunk_operational", "route_table",
    "ospf_neighbors", "port_security", "spanning_tree", "etherchannels",
    "mac_table", "arp_table", "device_facts", "device_inventory",
    "security_hardening", "wireless_hints", "vlan_brief", "acl_hit_counts",
    "interface_counters", "stp_root", "protocol_summary",
    "interface_features", "management_services", "gateway_redundancy",
    "routing_protocol_details", "dhcp_gateway_matches", "subnet_inventory",
    "policy_controls", "structured_relationships", "validation_findings",
    "endpoint_inventory", "access_tests", "client_access_matrix",
    "service_inventory", "roaming_events", "lab_result_summary",
}

EVIDENCE_ONLY_KEYS = {
    "command_blocks", "deep_evidence_index", "raw_evidence", "schema_map",
    "import_warnings", "all_config_commands", "native_pkt_profile", "native_source_manifest",
    "binary_evidence_summary", "binary_signatures", "recovered_string_preview",
    "native_conversion_guidance", "native_visible_hints", "internal_xml_bridge",
    "converted_xml_preview", "normalized_json_preview", "auto_conversion_pipeline",
    "decoded_payloads", "extraction_fidelity", "printable_segments_preview",
    "reconstructed_config_preview", "evidence_registry",
    "verified_extraction_contract", "companion_exports",
    "source_conversion_manifest", "source_payload_tree", "source_key_value_index",
    "universal_network_facts", "payload_tables", "universal_xml_preview",
    "universal_json_preview", "external_converter_outputs", "coverage_domains", "risk_atoms",
    "extraction_completeness",
}


def object_count_breakdown(objects):
    """Separate real topology/policy objects from support evidence.

    Older UI builds summed every parser row and accidentally made payload indexes,
    raw evidence previews, and native recovery manifests look like extracted network
    objects. This contract keeps the product honest while still surfacing the
    evidence generated by native PKT recovery and JSON/XML indexing.
    """
    counts = object_counts(objects)
    real_total = sum(int(counts.get(k, 0) or 0) for k in REAL_OBJECT_KEYS)
    evidence_total = sum(int(counts.get(k, 0) or 0) for k in EVIDENCE_ONLY_KEYS)
    routing_total = int(counts.get("routing", 0) or 0)
    if routing_total and "routing" not in REAL_OBJECT_KEYS:
        real_total += routing_total
    total = sum(int(v or 0) for v in counts.values())
    unknown_support = max(0, total - real_total - evidence_total)
    return {
        "real_object_count": real_total,
        "evidence_entry_count": evidence_total + unknown_support,
        "support_entry_count": evidence_total + unknown_support,
        "total_extracted_entries": total,
        "topology_object_count": real_total,
        "payload_index_count": int(counts.get("source_payload_tree", 0) or 0) + int(counts.get("source_key_value_index", 0) or 0) + int(counts.get("payload_tables", 0) or 0),
        "native_artifact_count": sum(int(counts.get(k, 0) or 0) for k in ["native_pkt_profile", "native_source_manifest", "binary_evidence_summary", "binary_signatures", "decoded_payloads", "internal_xml_bridge"]),
        "counts": counts,
        "explanation": "Topology/policy objects are counted separately from evidence rows, payload indexes, and native recovery artifacts.",
    }
...
def object_counts(objects):
    ...
    return {
        ...
        "all_config_commands": len(_safe_list(objects.get("all_config_commands"))),
        "interface_features": len(_safe_list(objects.get("interface_features"))),
        "management_services": len(_safe_list(objects.get("management_services"))),
        "gateway_redundancy": len(_safe_list(objects.get("gateway_redundancy"))),
        "routing_protocol_details": len(_safe_list(objects.get("routing_protocol_details"))),
        "extraction_completeness": len(_safe_list(objects.get("extraction_completeness"))),
        # Native Packet Tracer visibility counters. These are not simulated
        # topology objects; they are real backend evidence produced from the
        # uploaded .pkt/.pka binary and must be counted so the Import Center
        # never looks empty after a successful native intake.
        "native_pkt_profile": len(_safe_list(objects.get("native_pkt_profile"))),
        "native_source_manifest": len(_safe_list(objects.get("native_source_manifest"))),
        "binary_evidence_summary": len(_safe_list(objects.get("binary_evidence_summary"))),
        "binary_signatures": len(_safe_list(objects.get("binary_signatures"))),
        "recovered_string_preview": len(_safe_list(objects.get("recovered_string_preview"))),
        "native_conversion_guidance": len(_safe_list(objects.get("native_conversion_guidance"))),
        "native_visible_hints": len(_safe_list(objects.get("native_visible_hints"))),
        "internal_xml_bridge": len(_safe_list(objects.get("internal_xml_bridge"))),
        "converted_xml_preview": len(_safe_list(objects.get("converted_xml_preview"))),
        "normalized_json_preview": len(_safe_list(objects.get("normalized_json_preview"))),
        "auto_conversion_pipeline": len(_safe_list(objects.get("auto_conversion_pipeline"))),
        "decoded_payloads": len(_safe_list(objects.get("decoded_payloads"))),
        "extraction_fidelity": len(_safe_list(objects.get("extraction_fidelity"))),
        "printable_segments_preview": len(_safe_list(objects.get("printable_segments_preview"))),
        "reconstructed_config_preview": len(_safe_list(objects.get("reconstructed_config_preview"))),
        "verified_extraction_contract": len(_safe_list(objects.get("verified_extraction_contract"))),
        "companion_exports": len(_safe_list(objects.get("companion_exports"))),
        "source_conversion_manifest": len(_safe_list(objects.get("source_conversion_manifest"))),
        "source_payload_tree": len(_safe_list(objects.get("source_payload_tree"))),
        "source_key_value_index": len(_safe_list(objects.get("source_key_value_index"))),
        "universal_network_facts": len(_safe_list(objects.get("universal_network_facts"))),
        "payload_tables": len(_safe_list(objects.get("payload_tables"))),
        "universal_xml_preview": len(_safe_list(objects.get("universal_xml_preview"))),
        "universal_json_preview": len(_safe_list(objects.get("universal_json_preview"))),
        "evidence_registry": len(_safe_list(objects.get("evidence_registry"))),
        "external_converter_outputs": len(_safe_list(objects.get("external_converter_outputs"))),
    }
...
def build_timeline(state):
    ...
    for imp in state.get("imports", []):
        timeline.append({"time": imp.get("imported_at"), "type": "import", "severity": "Info", "detail": f"Imported {imp.get('filename')} with {imp.get('object_count', 0)} real objects and {imp.get('evidence_entry_count', 0)} support/evidence entries"})
    for d in build_policy_diff(state):
        if d["status"] != "Pass":
...
# ---------------------------------------------------------------------------
# v5.11 Unified Analyst Workspace helpers
# ---------------------------------------------------------------------------
...
def build_import_truth_summary(state):
    """Clear user-facing truth contract for imports and Packet Tracer limits."""
    state = _safe_dict(state)
    active = _safe_dict(state.get("active_extraction"))
    objects = get_objects(state)
    counts = object_counts(objects)
    breakdown = object_count_breakdown(objects)
    profile = _safe_dict(active.get("conversion_profile"))
    meta = _safe_dict(profile.get("metadata"))
    diagnostics = build_extraction_diagnostics(state)
    quality = build_evidence_quality_matrix(state)
    signoff = build_analyst_signoff(state)
    filename = active.get("filename") or "No file imported yet"
    source_mode = active.get("source_mode") or meta.get("source_mode") or "waiting"
    ext = (meta.get("extension") or Path(filename).suffix.replace('.', '') if filename else "").lower()
    native = ext in {"pkt", "pka"} or "pkt" in str(source_mode).lower()
    top_counts = []
    for key, label in [
        ("devices", "Devices"), ("interfaces", "Interfaces"), ("vlans", "VLANs"),
        ("dhcp_scopes", "DHCP scopes"), ("acl_rules", "ACL rules"), ("routing", "Routing"),
        ("cdp_links", "CDP links"), ("lldp_links", "LLDP links"),
        ("trunk_operational", "Trunks"), ("ip_inventory", "IP inventory"),
        ("native_source_manifest", "Native manifest"), ("binary_evidence_summary", "Binary evidence"),
        ("internal_xml_bridge", "XML bridge"), ("normalized_json_preview", "JSON preview"),
        ("raw_evidence", "Raw evidence"), ("evidence_registry", "Evidence registry"),
    ]:
        count = _count_value(counts, key)
        if count:
            top_counts.append({"key": key, "label": label, "count": count})
    critical_missing = []
    for key, label, why in [
        ("devices", "Device inventory", "Needed to name routers, switches, APs, and endpoints."),
        ("interfaces", "Interface inventory", "Needed for links, access/trunk mode, addressing, and shutdown state."),
        ("vlans", "VLAN database", "Needed for segmentation and trunk validation."),
        ("dhcp_scopes", "DHCP pools", "Needed to validate gateways and client reachability."),
        ("acl_rules", "ACL rules", "Needed to prove filtering and exposure."),
        ("routing", "Routes/protocols", "Needed for end-to-end reachability and root cause analysis."),
        ("cdp_links", "CDP/LLDP neighbor evidence", "Needed to confirm topology edges instead of inferring them."),
    ]:
        if _count_value(counts, key) == 0:
            critical_missing.append({"key": key, "label": label, "why": why})
    if not active:
        why = ["No evidence has been uploaded yet.", "Use the production sample to test the full workflow immediately."]
    elif native and critical_missing:
        why = [
            "Packet Tracer .pkt/.pka is a proprietary binary container.",
            "WiGuard can prove intake, hash, native inspection, and recovered evidence, but full topology needs exported companion evidence.",
            "Attach running-config, startup-config, show-command output, XML/JSON, or a ZIP bundle to upgrade hints into verified objects.",
        ]
    elif critical_missing:
        why = ["The uploaded evidence was parsed, but some required sections were not present or not recognizable.", "Add missing show-command/config sections so the parser can map line-level evidence."]
    else:
        why = ["Core topology and policy data are present. Review confidence before claiming full fidelity."]
    next_actions = []
    for action in diagnostics.get("recommended_actions", [])[:5]:
        if isinstance(action, dict):
            next_actions.append({"title": action.get("action", "Recommended action"), "why": action.get("why", "Improves extraction confidence."), "priority": action.get("priority", "P1")})
    if native:
        next_actions.insert(0, {"priority": "P0", "title": "Upload a Packet Tracer companion export", "why": "Use running-config/show outputs/XML/JSON to turn native binary hints into verified devices, links, VLANs, ACLs, and routes."})
    if not next_actions:
        next_actions = [
            {"priority": "P0", "title": "Import evidence", "why": "Upload a Packet Tracer/config/XML/JSON/ZIP file to start extraction."},
            {"priority": "P1", "title": "Open Workspace", "why": "Validate extracted objects and evidence mapping before reporting."},
        ]
    confidence = profile.get("readiness_score") or diagnostics.get("readiness_score") or quality.get("score") or 0
    return {
        "filename": filename,
        "source_mode": source_mode,
        "source_hash": active.get("source_hash") or meta.get("sha256") or "",
        "imported_at": active.get("imported_at") or "Not imported yet",
        "extension": ext or "unknown",
        "is_native_pkt": bool(native),
        "readiness_score": int(confidence or 0),
        "tier": diagnostics.get("tier") or profile.get("readiness") or "waiting",
        "claim": diagnostics.get("claim") or "Import evidence to calculate fidelity claims.",
        "can_claim_full_fidelity": bool(signoff.get("can_claim_full_fidelity")),
        "top_counts": top_counts,
        "count_breakdown": breakdown,
        "critical_missing": critical_missing,
        "why": why,
        "next_actions": next_actions[:6],
        "warnings": _safe_list(objects.get("import_warnings"))[:6],
        "raw_previews": _safe_list(objects.get("raw_evidence"))[:3],
        "bridge": (_safe_list(objects.get("internal_xml_bridge")) or [{}])[0],
        "quality": quality,
        "signoff": signoff,
    }


def build_workspace_model(state):
    """Single page model for topology + objects + evidence + timeline."""
    state = _safe_dict(state)
    objects = get_objects(state)
    counts = object_counts(objects)
    topology = build_topology(state)
    evidence_rows = _safe_list(objects.get("evidence_registry"))
    if not evidence_rows:
        synthesized = []
        for cat in ["devices", "interfaces", "vlans", "dhcp_scopes", "acl_rules", "route_table", "native_source_manifest", "binary_evidence_summary", "source_conversion_manifest", "universal_network_facts", "source_key_value_index", "payload_tables", "raw_evidence"]:
            for obj in _safe_list(objects.get(cat))[:20]:
                if isinstance(obj, dict):
                    ev = _evidence(obj)
                    name = obj.get("hostname") or obj.get("name") or obj.get("id") or obj.get("interface") or obj.get("vlan") or obj.get("command") or cat
                    synthesized.append({
                        "category": cat,
                        "name": str(name)[:160],
                        "status": "mapped" if ev else "review",
                        "confidence": ev.get("confidence", 0.45 if ev else 0.25),
                        "line": ev.get("source_line") or obj.get("evidence_line") or "—",
                        "source": ev.get("source_text") or obj.get("summary") or obj.get("preview") or "Object exists but evidence line needs review.",
                    })
        evidence_rows = synthesized
    tree_sections = []
    for key, label, icon in [
        ("devices", "Devices", "device"), ("interfaces", "Interfaces", "interface"),
        ("vlans", "VLANs", "vlan"), ("dhcp_scopes", "DHCP", "dhcp"),
        ("acl_rules", "ACL Rules", "acl"), ("route_table", "Routes", "route"),
        ("trunk_operational", "Trunks", "trunk"), ("cdp_links", "CDP Links", "link"),
        ("native_source_manifest", "Native PKT Evidence", "native"),
        ("source_conversion_manifest", "Source Conversion", "manifest"),
        ("universal_network_facts", "Universal Network Facts", "facts"),
        ("source_key_value_index", "Key/Value Index", "kv"),
        ("payload_tables", "Payload Tables", "tables"),
        ("raw_evidence", "Raw Evidence", "raw"),
    ]:
        rows = _safe_list(objects.get(key))
        tree_sections.append({"key": key, "label": label, "icon": icon, "count": len(rows), "rows": rows[:40]})
    return {
        ...
        "timeline": build_timeline(state)[:40],
        "objects_total": object_count_breakdown(objects).get("real_object_count", 0),
        "count_breakdown": object_count_breakdown(objects),
    }


def build_analysis_studio(state):
    """Unified model for policy diff, root cause, validation, simulation, playbooks."""
    state = _safe_dict(state)
    diffs = build_policy_diff(state)
    causes = build_root_causes(state)
    ...
    prioritized = sorted(diffs, key=lambda d: (severity_order.get(d.get("severity", "Info"), 9), 0 if d.get("status") == "Fail" else 1 if d.get("status") == "Review" else 2))
    severity_counts = {}
    for d in diffs:
        severity_counts[d.get("severity", "Info")] = severity_counts.get(d.get("severity", "Info"), 0) + 1
    rule_cards = [
        {"title": "Weak password / plain secrets", "coverage": "security_hardening", "status": "Active"},
        {"title": "Telnet enabled / SSH missing", "coverage": "management_services", "status": "Active"},
        {"title": "VLAN 1 / trunk drift", "coverage": "vlans + trunk_operational", "status": "Active"},
        {"title": "Duplicate IP / gateway gaps", "coverage": "ip_inventory + dhcp_scopes", "status": "Active"},
        {"title": "ACL too permissive", "coverage": "acl_rules", "status": "Active"},
        {"title": "Routing inconsistency", "coverage": "route_table + protocols", "status": "Active"},
    ]
    return {
        "risk": score,
        "diffs": diffs,
        ...
        "simulation_history": _safe_list(state.get("simulations"))[:10],
        "quality": build_evidence_quality_matrix(state),
        "truth": build_import_truth_summary(state),
    }


def build_report_builder_model(state, report_types=None):
    state = _safe_dict(state)
    truth = build_import_truth_summary(state)
    quality = build_evidence_quality_matrix(state)
    signoff = build_analyst_signoff(state)
    ...
    for key, title in report_types.items():
        if key == "executive": audience, reason = "Leadership", "Short risk, business impact, and top remediation priority."
        elif key == "technical": audience, reason = "Engineers", "Objects, topology, diff, evidence, and confidence."
        elif key == "packet_tracer": audience, reason = "Analysts", "Conversion fidelity, native limits, missing exports, and bridge output."
        elif key == "audit": audience, reason = "Auditors", "Hashes, manifest, line evidence, and integrity chain."
        elif key == "quality": audience, reason = "QA", "Evidence quality gate and sign-off controls."
        else: audience, reason = "Specialized", "Focused export for this workflow."
        blocked = key in {"executive", "full"} and not signoff.get("can_publish_executive")
        cards.append({"key": key, "title": title, "audience": audience, "reason": reason, "blocked": blocked})
    return {"truth": truth, "quality": quality, "signoff": signoff, "cards": cards}
